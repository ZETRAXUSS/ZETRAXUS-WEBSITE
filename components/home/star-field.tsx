...
const BASE_STARS = [
  ...
  [55, 88, 1.5], [66, 94, 2], [75, 84, 1], [85, 92, 1.5], [94, 82, 2],
...
// Aynı yıldız düzenini 3 farklı konuma kaydırarak 3 kat yıldız oluşturuyoruz.
// Mevcut yıldızların boyutları ve animasyonları korunuyor.
const STARS = [
  ...BASE_STARS,

  ...BASE_STARS.map(([x, y, size]) => [
    ((x + 3.7) % 100),
    ((y + 7.3) % 100),
    size,
  ]),
...
export function StarField() {
  ...
  useEffect(() => {
    ...
    const handlePointerLeave = () => resetStars();
    ...
    >
          ...
          style={
            {
              ...
              width: `${size}px`,
              height: `${size}px`,
              "--float-duration": `${5.5 + (index % 6) * 0.65}s`,
              "--float-delay": `${(index % 12) * 0.35}s`,
              "--twinkle-duration": `${2.2 + (index % 5) * 0.55}s`,
              "--twinkle-delay": `${(index % 9) * 0.3}s`,
      ...
      <style>{`
        ...
        .zx-star {
          ...
          opacity: calc(var(--star-opacity) * var(--twinkle));

          transform:
            translate3d(var(--escape-x), var(--escape-y), 0)
          ...
          animation:
            zx-star-float var(--float-duration, 6s) ease-in-out infinite,
            zx-star-twinkle var(--twinkle-duration, 3s) ease-in-out infinite;
